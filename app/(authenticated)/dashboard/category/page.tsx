...
} from "@/lib/server_actions/category";
...
export default function CategoryPage() {
	...
	const loadCategories = useCallback(async () => {
		...
		try {
			...
			const { data, count, error } = await fetchCategories({
	...
	useEffect(() => {
		...
		loadCategories();
	}, [currentPage, router, loadCategories]);
	...
	const handleArchive = async (id: string) => {
		...
		try {
			const { error } = await updateCategoryRecordStatus(id, {
			...
			if (error) {
				...
				const remainingRecords = categories.filter(
	...
	const handleUpdate = async (category: Category) => {
		try {
			...
			setCategories((prev) =>
				prev.map((elem) => (elem.id === category.id ? data[0] : elem)),
			);
		} catch (_error) {
			showErrorToast();
		} finally {
	...
	const handleServerError = (error: PostgrestError) => {
		if (error.message.includes(VALIDATION_ERRORS_MAPPING.serverError)) {
			showErrorToast(
				VALIDATION_ERRORS_MAPPING.entities.category.fields.name.displayError,
			);
		} else {
			showServerErrorToast(error.message);
		}
					...
					<div className="w-[1000px]">
						<div className="w-full md:text-right md:items-right mb-4">
							...
							<ExportExcel
								...
								records={[reportHeaders, ...categories].map((category, idx) => {
									return {
										row_no: idx > 0 ? idx : "Row No.",
										name: category.name,
										description: category.description,
										created_at: getDateWithoutTime(category.created_at),
									};
								})}
							/>
							<span className="px-1"></span>
								...
								{categories.map((category) => (
									<tr key={category.id} className="hover:bg-gray-50">
										...
										<td className="px-6 py-4 text-center">
											<span
												className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRecordStatusColor(category.status)}`}
											>
												{category.status}
											</span>
										</td>
