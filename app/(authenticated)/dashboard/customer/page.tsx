...
import {
	...
	saveCustomer,
	updateCustomer,
	updateCustomerRecordStatus,
} from "@/lib/server_actions/customer";
...
export default function CustomerPage() {
	...
	const loadCustomers = useCallback(async () => {
		...
		try {
			...
			const { data, count, error } = await fetchCustomers({
	...
	useEffect(() => {
		...
		loadCustomers();
	}, [router, loadCustomers, currentPage]);
	...
	const handleArchive = async (id: string) => {
		...
		try {
			const { error } = await updateCustomerRecordStatus(id, {
			...
			if (error) {
				...
				const remainingRecords = customers.filter(
	...
	const handleCreate = async (customer: Customer) => {
		...
		try {
			const { data, error } = await saveCustomer(customerWithNoId);
	...
	const handleUpdate = async (customer: Customer) => {
		try {
			...
			setCustomers((prev) =>
				prev.map((elem) => (elem.id === customer.id ? data[0] : elem)),
			);
		} catch (_error) {
			showErrorToast();
		} finally {
		...
		<div className="space-y-6">
			<div className="w-full md:w-5/6">
				<div className="md:flex md:justify-between md:items-center">
					...
					<AddButton label={"Add Customer"} handleAdd={handleAdd} />
				</div>
			</div>
					...
					<div className="w-[1000px]">
						<div className="w-full md:text-right md:items-right mb-4">
							...
							<ExportExcel
								...
								records={[reportHeaders, ...customers].map((customer, idx) => {
									return {
										...
										created_at: getDateWithoutTime(customer.created_at),
									};
								})}
							/>
							<span className="px-1"></span>
							...
							<tbody className="bg-white divide-y divide-gray-200">
								{customers.map((customer) => (
									<tr key={customer.id} className="hover:bg-gray-50">
										...
										>
											{canSeeMore
												? shortenText(customer.address, MAX_TABLE_TEXT_LENGTH)
												: customer.address}
											{customer.address &&
												customer.address.length > MAX_TABLE_TEXT_LENGTH && (
													<button
														type="button"
										...
										<td className="px-6 py-4 text-center">
											<span
												className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRecordStatusColor(customer.status)}`}
											>
												{customer.status}
											</span>
										</td>
