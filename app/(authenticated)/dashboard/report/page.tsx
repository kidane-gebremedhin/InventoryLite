...
export default function App() {
	...
	const [inventoryTurnoverReport, setInventoryTurnoverReport] = useState<
		InventoryTurnoverReport[]
	>([]);
	const [pendingOrdersReport, setPendingOrdersReport] = useState<
		PendingOrdersReport[]
	>([]);
	const [inventoryAgingReport, setInventoryAgingReport] = useState<
		InventoryAgingReport[]
	>([]);
	const [inventoryItems, setInventoryItems] = useState<InventoryItem[]>([]);
	const [selectedInventoryItemId, setSelectedInventoryItem] =
		useState(ALL_OPTIONS);
	const [orderNumber, setOrderNumber] = useState("");
	const [orderNumberTmp, setOrderNumberTmp] = useState("");
	const [fulfilledDateStart, setFulfilledDateStart] = useState<Date | null>(
		getCurrentDateTime(),
	);
	const [fulfilledDateEnd, setFulfilledDateEnd] = useState<Date | null>(
		getCurrentDateTime(),
	);
	...
	const [receivedDateStart, setReceivedDateStart] = useState<Date | null>(
		getCurrentDateTime(),
	);
	const [receivedDateEnd, setReceivedDateEnd] = useState<Date | null>(
		getCurrentDateTime(),
	);
	...
	const inventoryTurnoverReportHeaders = {
		item_name: "Inventory Item",
		sold_quantity: "Sale Quantity",
	};
	const pendingOrdersReportHeaders = {
		item_name: "Inventory Item",
		total_ordered_quantity: "Total Order Quantity",
		order_status: "Order Status",
	};
	const inventoryAgingReportHeaders = {
		item_name: "Inventory Item",
	...
	const reportTypes = [
		...
		{ name: "Canceled Sales Orders", type: ReportType.CANCELED_SALES_ORDERS },
		{
			...
			type: ReportType.PENDING_PURCHASE_ORDERS,
		},
		{
	...
	const loadInventoryItems = useCallback(async () => {
		try {
			...
			const selectableInventoryItems: InventoryItem[] = [
				{ id: ALL_OPTIONS, name: "" },
				...data,
			];
	...
	useEffect(() => {
		...
		const loadInventoryTurnoverReport = async () => {
			setLoading(
				canShowLoadingScreen(null, null, fulfilledDateStart, fulfilledDateEnd),
			);
			...
			try {
				const searchParams = {
					...
					...(selectedInventoryItemId && {
						selected_item_id: selectedInventoryItemId,
					}),
					...(fulfilledDateStart && {
						fulfilled_date_start: convertToUTC(
							setEarliestTimeOfDay(fulfilledDateStart),
						),
					}),
					...(fulfilledDateEnd && {
						fulfilled_date_end: convertToUTC(
							setLatestTimeOfDay(fulfilledDateEnd),
						),
					}),
				};
				const { data, error } = await makeRpcCall(
					RPC_FUNCTION.INVENTORY_TURNOVER,
					searchParams,
				);
				if (error) {
					showServerErrorToast(error.message);
	...
	useEffect(() => {
		...
		const rpcFunctionName = [
			...
			? RPC_FUNCTION.UNFULFILLED_SALES_ORDERS
			: RPC_FUNCTION.UNRECEIVED_PURCHASE_ORDERS;

		const loadOutstandingOrdersReport = async () => {
			setLoading(
				canShowLoadingScreen(expectedDateStart, expectedDateEnd, null, null),
			);
			...
			try {
				const searchParams = {
					...
					...(expectedDateStart && {
						expected_date_start: convertToUTC(
							setEarliestTimeOfDay(expectedDateStart),
						),
					}),
					...(expectedDateEnd && {
						expected_date_end: convertToUTC(
							setLatestTimeOfDay(expectedDateEnd),
						),
					}),
				};
				const { data, error } = await makeRpcCall(
	...
	useEffect(() => {
		...
		const loadInventoryAgingReport = async () => {
			setLoading(
				canShowLoadingScreen(null, null, receivedDateStart, receivedDateEnd),
			);
			...
			try {
				const searchParams = {
					...
					...(receivedDateStart && {
						received_date_start: convertToUTC(
							setEarliestTimeOfDay(receivedDateStart),
						),
					}),
					...(receivedDateEnd && {
						received_date_end: convertToUTC(
							setLatestTimeOfDay(receivedDateEnd),
						),
					}),
				};
				const { data, error } = await makeRpcCall(
					RPC_FUNCTION.INVENTORY_AGING,
					searchParams,
				);
				if (error) {
					showErrorToast();
	...
	const onFulfilledDateRangeChange = (dates) => {
	...
	const onExpectedDateRangeChange = (dates) => {
		const [start, end] = dates;
						...
						<div className="w-[1000px]">
							...
							<table className="min-w-full bg-white rounded-lg shadow-md">
								...
								<tbody className="divide-y divide-gray-200">
									{inventoryTurnoverReport.map((report) => {
										return (
											<tr key={report.item_id}>
							...
							<RecordsPerPage
								actualRecords={inventoryTurnoverReport.length}
						...
						<div className="w-[1000px]">
							<h2 className="text-2xl font-bold mb-4 text-gray-800">
								{
									reportTypes.find(
										(reportType) =>
											reportType.type.toString() === selectedReport,
									)?.name
								}
							</h2>
							<p className="mb-6 text-gray-600">
							...
							<div className="w-full md:text-right md:items-right mb-4">
								...
								<ExportExcel
									...
									records={[
										pendingOrdersReportHeaders,
										...pendingOrdersReport,
									].map((entry, idx) => {
										return {
											row_no: idx > 0 ? idx : "Row No.",
											item_name: entry.item_name,
											total_ordered_quantity: entry.total_ordered_quantity,
							...
							<table className="min-w-full bg-white rounded-lg shadow-md">
								...
								<tbody className="divide-y divide-gray-200">
									{pendingOrdersReport.map((report) => {
										return (
											<tr key={report.item_id}>
												<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
													{report.item_name}
							...
							<RecordsPerPage
								actualRecords={pendingOrdersReport.length}
								recordsPerPage={recordsPerPage}
								setRecordsPerPage={setRecordsPerPage}
							/>
						</div>
					...
					<div className="w-full overflow-x-scroll p-4">
						<div className="w-[1000px]">
								...
								<ExportExcel
									...
									records={[
										...
										return {
											...
											days_in_stock: entry.days_in_stock,
											received_date: entry.order_received_date,
										};
									})}
								/>
								<span className="px-1"></span>
							...
							<table className="min-w-full bg-white rounded-lg shadow-md">
								...
								<tbody className="divide-y divide-gray-200">
									{inventoryAgingReport.map((report) => {
										return (
											<tr key={report.item_id}>
												<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
													{report.item_name}
												...
												<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
													{formatDateToLocalDate(report.order_received_date)}
												</td>
											</tr>
										);
									})}
							...
							<RecordsPerPage
								actualRecords={inventoryAgingReport.length}
								recordsPerPage={recordsPerPage}
								setRecordsPerPage={setRecordsPerPage}
							/>
						</div>
