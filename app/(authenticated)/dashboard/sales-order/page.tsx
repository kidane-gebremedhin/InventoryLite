...
import {
	...
	saveSalesOrder,
	updateSalesOrder,
	updateSalesOrderRecordStatus,
	updateSalesOrderStatus,
} from "@/lib/server_actions/sales_order";
...
export default function SalesOrderPage() {
	...
	const [editingSalesOrder, setEditingSalesOrder] = useState<SalesOrder | null>(
		null,
	);
	...
	const [
		isMoveToFulfilledConfirmationModalOpen,
		setIsMoveToFulfilledConfirmationModalOpen,
	] = useState(false);
	const [
	...
	const loadCustomers = useCallback(async () => {
		fetchCustomerOptions(currentUser?.subscriptionInfo?.tenant_id).then(
			({ data, error }) => {
				if (error) return;

				const selectableCustomers: Customer[] = [
					{ id: ALL_OPTIONS, name: "" },
					...data,
				];
	...
	useEffect(() => {
		...
		loadSalesOrders();
	}, [currentPage, isInitialPageLoad, router, loadSalesOrders]);
	...
	const handleArchive = async (id: string) => {
		...
		try {
			const { error } = await updateSalesOrderRecordStatus(id, {
			...
			if (error) {
				...
				const remainingRecords = salesOrders.filter(
	...
	const handleRestore = async (id: string) => {
		...
		try {
			...
			if (error) {
				...
				setSalesOrders(
					salesOrders.filter((salesOrder) => salesOrder.id !== id),
				);
			}
		} catch (_error) {
			showErrorToast();
	...
	const handleCreate = async (salesOrder: SalesOrder) => {
		...
		try {
			...
			const { error } = await saveSalesOrder(payload);
			if (error) {
	...
	const handleUpdate = async (salesOrder: SalesOrder) => {
		...
		try {
			...
			const { error } = await updateSalesOrder(payload);
			if (error) {
				handleServerError(error);
				return;
			}
	...
	const handleOrderStatusChange = async (
		...
		try {
			const { error } = await updateSalesOrderStatus(id, {
				order_status: status,
			});
	...
	const getSalesOrderStatusOptions = () => [
		ALL_OPTIONS,
		...SALES_ORDER_STATUSES,
	];
	...
	const handleServerError = (error: PostgrestError) => {
		if (error.message.includes(VALIDATION_ERRORS_MAPPING.serverError)) {
			showErrorToast(
				VALIDATION_ERRORS_MAPPING.entities.sales_order.fields.name.displayError,
			);
		} else {
			showServerErrorToast(error.message);
		}
	...
	const getReportFields = (salesOrder, idx: number) => {
		return salesOrder.order_items?.map((orderItem, innerIdx) => {
			...
			return {
				...
				fulfilled_date: showRowNo
					? getDateWithoutTime(salesOrder.fulfilled_date)
					: "",
				...
				<div className="w-full overflow-x-scroll p-4">
					<div className="w-[1050px]">
						<div className="w-full md:text-right md:items-right mb-4">
							...
							<ExportExcel
								reportName="Sales Orders"
								records={[
									reportHeaders,
									...salesOrders.flatMap((salesOrder, idx) =>
										getReportFields(salesOrder, idx),
									),
								]}
							/>
							<span className="px-1"></span>
							...
							<tbody className="bg-white divide-y divide-gray-200">
								{salesOrders.map((order) => (
									<tr key={order.id} className="hover:bg-gray-50">
										...
										<td className="px-6 py-4">
											<span
												className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getOrderStatusColor(order.order_status)}`}
											>
												{order.order_status}
											</span>
										</td>
												...
												<ActionsMenu
													actions={[
														...
														},
														{
															id: order.id,
															hideOption:
																![SalesOrderStatus.CANCELED].includes(
																	order.order_status,
														...
														},
														{
															id: order.id,
															hideOption:
																[SalesOrderStatus.FULFILLED].includes(
																	order.order_status,
																) || selectedStatus === RecordStatus.ARCHIVED,
														...
														},
														{
															id: order.id,
															hideOption:
																![SalesOrderStatus.PENDING].includes(
																	order.order_status,
																) || selectedStatus === RecordStatus.ARCHIVED,
