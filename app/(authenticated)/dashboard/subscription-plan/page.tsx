...
import { ALL_OPTIONS, CURRENCY_TYPES, FIRST_PAGE_NUMBER, MAX_DROPDOWN_TEXT_LENGTH, MAX_TABLE_TEXT_LENGTH, RECORD_STATUSES, RECORDS_PER_PAGE, SUBSCRIPTION_STATUSES, TEXT_SEARCH_TRIGGER_KEY, VALIDATION_ERRORS_MAPPING } from '@/lib/Constants'
...
import { fetchSubscriptionPlans, saveSubscriptionPlan, updateSubscriptionPlan, updateSubscriptionPlanRecordStatus } from '@/lib/server_actions/subscription_plan'
...
export default function SubscriptionPlanPage() {
  ...
  const reportHeaders = { subscription_status: 'Subscription Status', currency_type: 'Currency', payment_amount: 'Payment Amount', created_at: 'Date Created' }

  useEffect(() => {
    // reset pagination
    router.push(`?page=${currentPage}`)
    loadSubscriptionPlans()
  }, [selectedSubscriptionStatus, selectedCurrencyType, selectedStatus, recordsPerPage, currentPage])
  ...
  const loadSubscriptionPlans = async () => {
    ...
    try {
      ...
      const { data, count, error } = await fetchSubscriptionPlans({ selectedSubscriptionStatus, selectedCurrencyType, selectedStatus, startIndex, endIndex });
  ...
  const handleEdit = (id: string) => {
    const subscriptionPlan = subscriptionPlans.find(subscriptionPlan => subscriptionPlan.id === id)
  ...
  const handleArchive = async (id: string) => {
    ...
    try {
      const { error } = await updateSubscriptionPlanRecordStatus(id, { status: RecordStatus.ARCHIVED })

      if (error) {
        showServerErrorToast(error.message)
      } else {
        showSuccessToast('Record Archived.')
        const remainingRecords = subscriptionPlans.filter(subscriptionPlan => subscriptionPlan.id !== id)
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      const { error } = await updateSubscriptionPlanRecordStatus(id, { status: RecordStatus.ACTIVE })
  ...
  const handleCreate = async (subscriptionPlan: SubscriptionPlan) => {
    ...
    try {
      const { data, error } = await saveSubscriptionPlan(subscriptionPlanWithNoId)
  ...
  const handleUpdate = async (subscriptionPlan: SubscriptionPlan) => {
    try {
      const { data, error } = await updateSubscriptionPlan(subscriptionPlan.id, subscriptionPlan)
      ...
      setSubscriptionPlans(prev => prev.map(elem => elem.id === subscriptionPlan.id ? data[0] : elem))
    } catch (error: any) {
      showErrorToast()
    } finally {
      setLoading(false)
  ...
  const handleServerError = (error: PostgrestError) => {
    if (error.message.includes(VALIDATION_ERRORS_MAPPING.serverError)) {
      showErrorToast(VALIDATION_ERRORS_MAPPING.entities.subscriptionPlan.fields.subscription_status.displayError)
    } else {
      showServerErrorToast(error.message)
    }
  }
      ...
      <div className="w-full md:w-5/6">
        <div className="md:flex md:justify-between md:items-center">
          <div>
            ...
            <p className="text-gray-600">Manage subscription Subscription plans</p>
          </div>
          <button
            onClick={handleAdd}
            className="w-full md:w-1/4 btn-primary flex justify-center items-center"
          >
        ...
        <div className="w-full overflow-x-scroll p-4">
          <div className="w-[1000px]">
            <div className="w-full md:text-right md:items-right mb-4">
              ...
              <ExportExcel reportName="subscription_plans" records={[reportHeaders, ...subscriptionPlans].map((subscriptionPlan, idx) => {
                return { row_no: idx > 0 ? idx : 'Row No.', subscription_status: subscriptionPlan.subscription_status, currency_type: subscriptionPlan.currency_type, payment_amount: subscriptionPlan.payment_amount, created_at: getDateWithoutTime(subscriptionPlan.created_at) }
              })} />
              <span className="px-1"></span>
              <ExportPDF reportName="subscription_plans" records={[reportHeaders, ...subscriptionPlans].map((subscriptionPlan, idx) => {
                return { row_no: idx > 0 ? idx : 'Row No.', subscription_status: subscriptionPlan.subscription_status, currency_type: subscriptionPlan.currency_type, payment_amount: subscriptionPlan.payment_amount, created_at: getDateWithoutTime(subscriptionPlan.created_at) }
              })} />
            </div>
                ...
                {subscriptionPlans.map((subscriptionPlan) => (
                  <tr key={subscriptionPlan.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{subscriptionPlan.subscription_status}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{subscriptionPlan.currency_type}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{subscriptionPlan.payment_amount}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRecordStatusColor(subscriptionPlan.status!)}`}>
