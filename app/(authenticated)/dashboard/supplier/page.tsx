...
import {
	...
	saveSupplier,
	updateSupplier,
	updateSupplierRecordStatus,
} from "@/lib/server_actions/supplier";
...
export default function SupplierPage() {
	...
	useEffect(() => {
		...
		loadSuppliers();
	}, [router, currentPage, loadSuppliers]);
	...
	const handleArchive = async (id: string) => {
		...
		try {
			const { error } = await updateSupplierRecordStatus(id, {
			...
			if (error) {
				...
				const remainingRecords = suppliers.filter(
	...
	const handleUpdate = async (supplier: Supplier) => {
		try {
			...
			setSuppliers((prev) =>
				prev.map((elem) => (elem.id === supplier.id ? data[0] : elem)),
			);
		} catch (_error) {
			showErrorToast();
		} finally {
		...
		<div className="space-y-6">
			<div className="w-full md:w-5/6">
				<div className="md:flex md:justify-between md:items-center">
					...
					<AddButton label={"Add Supplier"} handleAdd={handleAdd} />
				</div>
			</div>
				...
				<div className="w-full overflow-x-scroll p-4">
					<div className="w-[1000px]">
						<div className="w-full md:text-right md:items-right mb-4">
							...
							<ExportExcel
								reportName="Supplieers"
								records={[reportHeaders, ...suppliers].map((supplier, idx) => {
									return {
										...
										created_at: getDateWithoutTime(supplier.created_at),
									};
								})}
							/>
							<span className="px-1"></span>
							...
							<tbody className="bg-white divide-y divide-gray-200">
								{suppliers.map((supplier) => (
									<tr key={supplier.id} className="hover:bg-gray-50">
										...
										>
											{canSeeMore
												? shortenText(supplier.address, MAX_TABLE_TEXT_LENGTH)
												: supplier.address}
											{supplier.address &&
												supplier.address.length > MAX_TABLE_TEXT_LENGTH && (
													<button
														type="button"
										...
										<td className="px-6 py-4 text-center">
											<span
												className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRecordStatusColor(supplier.status)}`}
											>
												{supplier.status}
											</span>
										</td>
