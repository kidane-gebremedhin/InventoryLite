...
import { UserInvitation } from '@/lib/types/Models'
import { InvitationStatus, RecordStatus } from '@/lib/Enums'
import { ALL_OPTIONS, FIRST_PAGE_NUMBER, INVITATION_STATUSES, RECORDS_PER_PAGE, TEXT_SEARCH_TRIGGER_KEY, VALIDATION_ERRORS_MAPPING } from '@/lib/Constants'
import { calculateStartAndEndIndex, formatDateToLocalDate, getDateWithoutTime, getInvitationStatusColor, showErrorToast, showServerErrorToast, showSuccessToast } from '@/lib/helpers/Helper'
...
export default function UserInvitationPage() {
  ...
  const [editingUserInvitation, setEditingUserInvitation] = useState<UserInvitation | null>(null)
  const [showFilters, setShowFilters] = useState(false)
  const [selectedStatus, setSelectedStatus] = useState(InvitationStatus.OPEN.toString())
  // Pagination
  ...
  const [isExpireConfirmationModalOpen, setIsExpireConfirmationModalOpen] = useState(false)
  const [isOpenConfirmationModalOpen, setIsOpenConfirmationModalOpen] = useState(false)
  // Global States
  const { loading, setLoading } = useLoadingContext()

  const reportHeaders = { email: 'Email', status: 'Status', created_at: 'Date Created', expires_at: 'Expiration Date' }
  ...
  const loadUserInvitations = async () => {
    ...
    try {
      ...
      const { data, count, error } = await fetchUserInvitations({ selectedStatus, searchTerm, startIndex, endIndex });
  ...
  const handleEdit = (id: string) => {
    const userInvitation = userInvitations.find(userInvitation => userInvitation.id === id)
  ...
  const handleExpire = async (id: string) => {
    ...
    try {
      const { error } = await updateUserInvitationStatus(id, { status: InvitationStatus.EXPIRED })

      if (error) {
        showServerErrorToast(error.message)
      } else {
        showSuccessToast('Invitation Expired.')
        const remainingRecords = userInvitations.filter(userInvitation => userInvitation.id !== id)
  ...
  const handleOpen = async (id: string) => {
    ...
    try {
      const { error } = await updateUserInvitationStatus(id, { status: InvitationStatus.OPEN })
  ...
  const handleCreate = async (userInvitation: Partial<UserInvitation>) => {
    ...
    try {
      const { data, error } = await saveUserInvitation(userInvitationWithNoId)
      if (error) {
  ...
  const handleUpdate = async (userInvitation: Partial<UserInvitation>) => {
    try {
      const { data, error } = await updateUserInvitation(userInvitation.id, userInvitation)
      ...
      setUserInvitations(prev => prev.map(elem => elem.id === userInvitation.id ? data[0] : userInvitation))
    } catch (error: any) {
      showErrorToast()
    } finally {
      setLoading(false)
  ...
  const handleServerError = (error: PostgrestError) => {
    if (error.message.includes(VALIDATION_ERRORS_MAPPING.serverError)) {
      showErrorToast(VALIDATION_ERRORS_MAPPING.entities.userInvitation.fields.email.displayError)
    } else {
      showServerErrorToast(error.message)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">User Invitation Management</h1>
      ...
      <div className="card">
        <div className="overflow-x-auto">
          <div className="w-full text-right items-right mb-4">
            ...
            <ExportExcel reportName="UserInvitations" records={[reportHeaders, ...userInvitations].map((userInvitation, idx) => {
              return { row_no: idx > 0 ? idx : 'Row No.', email: userInvitation.email, status: userInvitation.status, created_at: getDateWithoutTime(userInvitation.created_at), expires_at: getDateWithoutTime(userInvitation.expires_at) }
            })} />
            <span className="px-1"></span>
            <ExportPDF reportName="UserInvitations" records={[reportHeaders, ...userInvitations].map((userInvitation, idx) => {
              return { row_no: idx > 0 ? idx : 'Row No.', email: userInvitation.email, status: userInvitation.status, created_at: getDateWithoutTime(userInvitation.created_at), expires_at: getDateWithoutTime(userInvitation.expires_at) }
            })} />
          </div>
              ...
              {userInvitations.map((userInvitation) => (
                <tr key={userInvitation.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div>
                      <div className="text-sm font-medium text-gray-900">{userInvitation.email}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getInvitationStatusColor(userInvitation.status!)}`}>
                      {userInvitation.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-900">
                    {getDateWithoutTime(formatDateToLocalDate(userInvitation.created_at))}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-900">
                    {getDateWithoutTime(formatDateToLocalDate(userInvitation.expires_at))}
                  </td>
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    <div className="flex justify-center space-x-2 items-center">
                      {userInvitation.status !== InvitationStatus.ACCEPTTED && (
                        <ActionsMenu
                          actions={[
                            {
                              id: userInvitation.id!,
                              hideOption: selectedStatus !== InvitationStatus.OPEN,
                            ...
                            {
                              id: userInvitation.id!,
                              hideOption: selectedStatus !== InvitationStatus.EXPIRED,
