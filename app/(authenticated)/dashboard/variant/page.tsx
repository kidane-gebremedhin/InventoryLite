...
import { Variant } from '@/lib/types/Models'
...
import { fetchVariants, saveVariant, updateVariant, updateVariantRecordStatus } from '@/lib/server_actions/variant'
...
export default function VariantPage() {
  ...
  const reportHeaders = { name: 'Variant Name', description: 'Description', created_at: 'Date Created' }
  ...
  const loadVariants = async () => {
    ...
    try {
      ...
      const { data, count, error } = await fetchVariants({ selectedStatus, searchTerm, startIndex, endIndex });
  ...
  const handleArchive = async (id: string) => {
    ...
    try {
      const { error } = await updateVariantRecordStatus(id, { status: RecordStatus.ARCHIVED })

      if (error) {
        showServerErrorToast(error.message)
      } else {
        showSuccessToast('Record Archived.')
        const remainingRecords = variants.filter(variant => variant.id !== id)
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      const { error } = await updateVariantRecordStatus(id, { status: RecordStatus.ACTIVE })
  ...
  const handleUpdate = async (variant: Variant) => {
    try {
      ...
      setVariants(prev => prev.map(elem => elem.id === variant.id ? data[0] : variant))
    } catch (error: any) {
      showErrorToast()
    } finally {
      setLoading(false)
  ...
  const handleServerError = (error: PostgrestError) => {
    if (error.message.includes(VALIDATION_ERRORS_MAPPING.serverError)) {
      showErrorToast(VALIDATION_ERRORS_MAPPING.entities.variant.fields.name.displayError)
    } else {
      showServerErrorToast(error.message)
    }
  }
      ...
      <div className="card">
        <div className="overflow-x-auto">
          <div className="w-full text-right items-right mb-4">
            ...
            <ExportExcel reportName="Variants" records={[reportHeaders, ...variants].map((variant, idx) => {
              return { row_no: idx > 0 ? idx : 'Row No.', name: variant.name, description: variant.description, created_at: getDateWithoutTime(variant.created_at) }
            })} />
            <span className="px-1"></span>
            <ExportPDF reportName="Variants" records={[reportHeaders, ...variants].map((variant, idx) => {
              return { row_no: idx > 0 ? idx : 'Row No.', name: variant.name, description: variant.description, created_at: getDateWithoutTime(variant.created_at) }
            })} />
          </div>
              ...
              {variants.map((variant) => (
                <tr key={variant.id} className="hover:bg-gray-50">
                  ...
                  <td style={{ maxWidth: 200 }} className="px-6 py-4 text-sm text-gray-900 o">
                    {canSeeMore ? shortenText(variant.description, MAX_TABLE_TEXT_LENGTH) : variant.description}
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRecordStatusColor(variant.status!)}`}>
                      {variant.status}
                    </span>
                  </td>
