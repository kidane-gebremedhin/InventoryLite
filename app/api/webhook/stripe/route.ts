...
import { DATABASE_TABLE, STRIPE_PAYMENT_EVENT } from "@/lib/Enums";
...
export async function POST(req) {
	...
	const signature: string | null = requestHeaders.get("stripe-signature");
	...
	try {
		switch (eventType) {
			case STRIPE_PAYMENT_EVENT.CHECKOUT_SESSION_COMPLETED: {
				...
				await supabase.from(DATABASE_TABLE.tenants).insert({
					name: `${priceId} ${customerId}`,
					domain: `${customer.email} ${customer.name}`,
					status: "active",
					price_id: `${eventType} ${customer.email}`,
				});
				/*
                if (customer.email) {
