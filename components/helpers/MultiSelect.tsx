...
const MultiSelect = ({ options, preSelectedValues, changeHandler }: Props) => {
    ...
    // Toggles the dropdown and clears the search term if closing
    const handleToggle = () => {
        setIsOpen(!isOpen);
        ...
        <div className="flex items-center justify-center bg-gray-50">
            ...
            >
                <div className="relative z-10">
                    ...
                    <div
                        className={`flex flex-wrap items-center min-h-[44px] border ${isOpen ? 'border-indigo-500 ring-2 ring-indigo-200' : 'border-gray-300'
                        ...
                        <input
                            ...
                            onKeyDown={(e) => {
                                ...
                                if (e.key === 'Backspace' && searchTerm === '' && selectedOptions.length > 0) {
                                    e.preventDefault();
                                    handleDeselect(selectedOptions[selectedOptions.length - 1].id);
                                }
                            }}
                            className="flex-grow min-w-[100px] p-1 bg-transparent focus:outline-none placeholder-gray-500 text-gray-800"
                            placeholder={selectedOptions.length === 0 ? "Select Variants..." : "Search or add..."}
                        />
                    </div>
                            ...
                            {availableOptions.length > 0 ? (
                                availableOptions.map(option => (
                                    ...
                                    >
                                        ...
                                        {selectedOptions.some(o => o.id === option.id) && <Check className="w-4 h-4 text-indigo-500" />}
                                    </div>
                                ))
                            ) : (
                                <div className="p-3 text-gray-500 text-center">
                                    {searchTerm ? `No results found for "${searchTerm}"` : 'All options selected'}
                                </div>
                            )}
                        </div>
                    )}
