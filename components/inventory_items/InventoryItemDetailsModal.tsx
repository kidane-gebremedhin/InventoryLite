						...
						<div className="card">
							...
							<div className="space-y-2 text-sm">
								...
								{item.item_variants?.map((iv) => (
									<span
										key={iv.id}
										className="ml-2 px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800"
									>
										{iv.variant?.name}
									</span>
								))}
								<div>
									...
									<span
										className={`ml-2 px-2 py-1 text-xs font-semibold rounded-full ${getRecordStatusColor(item.status)}`}
									>
										{item.status}
									</span>
								</div>
								<div>
									...
									<span className="ml-2">
										{formatDateToLocalDate(item.created_at)}
									</span>
								</div>
								<div>
									<span className="text-gray-600">Updated:</span>
									<span className="ml-2">
										{formatDateToLocalDate(item.updated_at)}
									</span>
								</div>
							</div>
						</div>
						...
						<div className="card">
							...
							<div className="space-y-2 text-sm">
								<div>
									...
									<span className="ml-2 font-medium">
										{item.purchase_order_items?.length}
									</span>
								</div>
								<div>
