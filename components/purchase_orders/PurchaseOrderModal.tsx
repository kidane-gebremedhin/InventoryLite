...
export default function PurchaseOrderModal({
	...
	const updateItem = (index: number, field: keyof PurchaseOrderItem, value) => {
		if (value && field === "unit_price" && !DECIMAL_REGEX.test(value)) {
			return;
		}

		const preSelectedItem = purchaseOrderItems.find(
					...
					<div>
						...
						{purchaseOrderItems.length === 0 ? (
							<div className="text-center py-8 text-gray-500">
								No purchase order items added yet. Click "Add Item" to get
								started.
							</div>
						) : (
							<div className="space-y-4">
								...
								<div className="text-right">
									<p className="text-lg font-medium">
										Total Value:{" "}
										{calculateOrderTotalProce(purchaseOrderItems).toFixed(2)}
									</p>
								</div>
							</div>
						)}
