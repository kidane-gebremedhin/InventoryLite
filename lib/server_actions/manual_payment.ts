...
import type { ManualPayment, ServerActionsResponse } from "../types/Models";
...
export async function declineManualPayment(
	id: string,
): Promise<ServerActionsResponse> {
	const supabase = await createClient();
...
export async function saveManualPayment(
	requestData: ManualPayment,
): Promise<ServerActionsResponse> {
	const supabase = await createClient();
